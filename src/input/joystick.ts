 ...
 */
export function readJoystick(dx: number, dy: number, radius: number, deadZone: number): JoystickReading {
  const distance = Math.hypot(dx, dy);
  const scale = distance > radius ? radius / distance : 1;
  const knob = { x: Math.round(dx * scale), y: Math.round(dy * scale) };
  if (distance < radius * deadZone || distance === 0) return { direction: { x: 0, y: 0 }, knob };
