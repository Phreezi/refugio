...
- **Tamanho da interface** (definições, preferência do dispositivo `uiSize`: Grande/Médio/Pequeno): a interface (HUD, menus, painéis) tem uma vista própria (`getView()`) com zoom inteiro um ou dois níveis abaixo do do mundo (`uiZoomFor`: nunca abaixo de metade, nem com o lado curto da interface acima de 560 px de jogo); o mundo usa `getWorldView()` e os textos da ZoneScene seguem o zoom do mundo. A câmara fixa (`setupFixedCamera`) tem origem no canto.
...
8. **Versão**: `package.json` → `version` = `0.0.<número do PR>` (ex.: o PR 53 é a v0.0.53), até se definir outra numeração. Atualizar em cada PR (vê-se no menu inicial).
...
- Comer cura um pouco: a comida sem efeito próprio na vida dá `foodHealPct`% (25%) do que mata a fome (`healOf`; a carne crua continua a tirar vida).
...
| Andar agachado (metade da velocidade) | Ctrl | Joystick pouco empurrado (até 55% do raio) |
| Correr (`sprintMultiplier` ×; fome e sede gastam-se `sprintDrainMultiplier`× mais depressa) | Shift: toque liga/desliga, segurar corre enquanto se segura | Botão "Correr" (idem) |
| Zoom (até metade) | Ctrl + roda do rato, +/− | Pinça com 2 dedos |
| Velocidade do jogo x1/x2/x3 | Botão por baixo do relógio | Idem |
| Mapa do mundo (onde estou; arrasta-se para ver o resto; mostra a mochila caída e as pilhas largadas, os postes — azul = ativo —, NPCs com missão "!"/"?", chefes por derrotar e, a verde, as zonas-destino das missões) | M | Botão "Mapa" (à esquerda do "II") |
| Pausa (definições, estatísticas, sair) | Esc (sem painéis abertos) / botão "II" | Botão "II" |
| Ação contextual (bater, recolher, abrir, atacar) | Espaço / clique (manter premido repete golpes ao ritmo da arma); com uma pilha/contentor aberto, Espaço outra vez = "Apanhar tudo" | Botão grande (lado direito) |
| Voltar a casa (parado `recallSec` = 5 s; andar ou levar dano interrompe; outra vez cancela) | H | Botão "Casa" (por baixo do "Mapa") |
| Opções do Auto: atacar e/ou apanhar (`autoFight`/`autoGather`, preferências do dispositivo) | Segurar o botão "Auto" (ou o rato em cima dele) | Segurar o botão "Auto" |
| Ataque automático (liga/desliga): sem a ação premida, bate ou dispara sozinho no inimigo mais perto ao alcance da arma (com linha de vista); sem inimigos, recolhe sozinho o recurso à frente. Com o Auto ligado, a ação manual apanha/abre o que estiver à frente mesmo a meio de um combate | F / botão "Auto" (canto inferior direito) | Botão "Auto" (por cima do botão de ação) |
...
- Grelha de slots. Base 20, mais os `slots` da mochila equipada (slot "Moch." na coluna do equipamento): bolsa +5 → mochila pequena +10 → grande +20 → militar +30 → expedição +40 (nível 30) → sobrevivente +50 (nível 45); cada uma fabrica-se a partir da anterior. Ao tirar a mochila, o que estava nos espaços dela passa para espaços livres; o que não couber fica numa pilha no chão (`PlayerActions.syncBackpack`, a cada tick). Com muitos espaços, a grelha do painel ganha colunas para caber no ecrã.
- Stacks: recursos 50, consumíveis 10, munições 100 (flechas, virotes e seixos 999), ferramentas/armas 1.
- Slots de equipamento: arma, cabeça, corpo, pernas, pés, mochila. A **arma** vê-se também num 5.º slot ao lado da hotbar (fundo mais escuro, só para armas): arrastar para lá equipa; tocar passa à munição seguinte (a munição em uso e quantas há aparecem por cima).
...
- **Moedas** não ocupam slots: são um contador (`player.coins`, no HUD ao lado da barra de XP). Apanhar moedas (drops, loot, corpos) soma ao contador; se uma for arrastada para um slot, passa para o contador no tick seguinte (`PlayerActions.sweepCoins`). As receitas pagam/recebem moedas do contador (`Wallet` em `systems/crafting`).
- **Toque duplo** num slot com um baú, contentor, corpo ou pilha aberto passa-o para o outro lado (`PlayerActions.quickMove`: junta às pilhas iguais, depois espaços vazios; avisa se não couber).
- Tocar num **ingrediente** (fabrico, o que o técnico pede, objetivos das missões): se der para o fazer ali (nas mãos ou na estação aberta), faz 1; senão diz onde se arranja — receitas, inimigos, recursos, contentores e lojas (`src/ui/itemSources.ts`).
- Fabricar com a mochila cheia: os ingredientes saem primeiro, por isso gastar um 1/1 abre espaço para o produto; se não couber, não se faz ("Mochila cheia"). Nas estações o produto espera na estação.
...
- **Mira presa**: com a ação premida, os tiros seguem o mesmo inimigo (enquanto estiver vivo e ao alcance); ao largar, o próximo tiro vai ao mais perto. Armas à distância leves no início: **fisga** (seixos; 1 pedra → 6), **arco curto** (flechas; 1 madeira → 5), **arco de caça** (bancada, nível 7); depois a besta e a pistola. Flechas de **pedra** (+3 de dano, partem 35%) e de **ferro** (+6, partem 15%); a de madeira parte 60% (`ammoOf`, `ammoDamage`, `breakPct` em `items.json`). Ao equipar entra primeiro a que dá mais dano; a primeira da aljava é a que está em uso (tocar na arma no HUD, ou na aljava da mochila, passa à seguinte) e, quando acaba, passa-se à mais forte. Falha-se menos perto do alvo (`missNearFactor` encostado, a falha toda no alcance máximo).
- **Aljava** (`player.quiver`, save v16): com uma arma à distância equipada, a munição que lhe serve entra "dentro" dela (não ocupa slots, sem limite); ao trocar de arma volta para a mochila e o que não couber fica numa pilha no chão. O HUD mostra a arma equipada, a munição em uso e quantas há por cima da hotbar (no máximo "999"); no painel da mochila, o total aparece no canto superior direito do slot da arma. Arrastar um item para fora do painel da mochila larga-o no chão.
- **Linha de vista**: a mira e o ataque automático só apontam a inimigos sem paredes/obstáculos pelo meio (`Combat.canSee`).
- Os avisos repetidos ("Sem munição!") desaparecem em `SHORT_NOTICE_MS` (1,2 s); com uma conversa aberta, os avisos (e os prémios "+N item") aparecem por baixo do painel.
- **Corpos**: um inimigo derrotado fica no chão, na posição em que estava e **a cinzento** (`display/grayTexture.ts`), com os **drops** e as flechas que lhe acertaram e não partiram. É uma pilha no chão (`zones.<zona>.bags` com `corpse: <inimigo>`, save v20): abre-se com a ação ("Apanhar tudo" ou escolher) e, vazio, desaparece numa nuvem de fumo. Fica `groundLootHours` (6) horas de jogo, também se o jogador sair da zona (as pilhas largadas também). No máximo `groundThingsMax` (300) coisas no chão por zona (corpos, pilhas, itens soltos): as mais antigas desaparecem (as mochilas da morte nunca).
- **Proteção de principiante**: até às 00:00 do dia `beginnerUntilDay` (4) nada se gasta — armas, ferramentas, cana e armadura (aviso no HUD).
- Sem munição, disparar (ou o Auto, de `noAmmoWarnSec` em `noAmmoWarnSec`) avisa "Sem munição!".
- **Perícias** (`player.skills`, `systems/combat/skills.ts`): punhos, impacto, lâminas, pontaria, armas de fogo (`skill` em `items.json`; omisso: pontaria à distância, impacto corpo a corpo). Cada golpe ou tiro dá 1 de experiência (curva `skillCurve`, até `skillMaxLevel`) e sorteia se falha: `missPctMelee` (12%) − `missPctPerLevel` por nível, ou `missPctRanged` (60%) − `missPctPerLevelRanged` à distância, nunca abaixo de `missPctMin`. Um golpe falhado mostra "Falhou"; um tiro falhado sai desviado (`missSpreadDeg`).
- **Munição recuperável** (`recoverable`: flechas, virotes, seixos): o tiro que não acerta em ninguém fica no chão onde parou (pode partir: metade do `breakPct`) (`zones.<zona>.ground = [[x, y, item, qtd]]`, até `groundItemsMax`) e apanha-se ao passar por cima (`groundPickupPx`).
...
- **Talentos** (`src/data/talents.json`, lógica em `systems/progression/talents.ts`): cada nível dá 1 ponto (pontos = nível − 1 − gastos; quem já jogava fica com os pontos todos). 3 ramos com 8 talentos (Combate, Sobrevivência, Ofícios), até ao nível 35 e alguns com 50 pontos (há sempre onde gastar); cada talento tem `maxRank`, `level` mínimo e `requires: [talento, pontos]` do mesmo ramo. Efeitos (`effect` × `perRank`): `meleeDamagePct`, `rangedDamagePct`, `missPts`, `armorPct` (soma à armadura, até ao máximo), `hungerSlowPct`, `thirstSlowPct`, `regenPct`, `sprintCostPct` ("Fôlego": correr gasta menos), `xpPct`, `gatherPower`, `extraDropPct`, `wearSavePct` e `craftSpeedPct` (estes dois até `talentCapPct`; o fabrico conta ao pôr na fila).
...
- **Moedas** (`coin`; um contador do jogador, não ocupam slots — §7.3): caem dos inimigos (`drops` em `enemies.json`) e aparecem em contentores (`lootTables.json`).
...
- O HUD mostra as moedas ao lado da barra de XP. Nos ingredientes vê-se quanto se tem mesmo ("42/3 Moeda", "410/10 Madeira"); as vendas mostram-se como "Madeira 410/10 (2× Moeda)", com o ícone do recurso. No fabrico, as receitas de nível acima aparecem no fim, a cinzento, com os ingredientes.

### 7.17 Frigorífico, take-away e efeitos da comida

- **Frigorífico** (`fridge`, peça construída na base, nível 15, muitos materiais: ferro, sucata, eletrónica, químicos, tábuas, pregos): um baú de `chestSlots` (60) espaços que **só aceita comida e bebida** (`foodOnly`: itens com fome ou sede; arrastar outra coisa avisa).
- **Encomendar** (botão no painel do frigorífico, `orders: "takeaway"`): abre a estação virtual `takeaway` (`deliver: true`, fila de 3) com as receitas `category: "order"` (refeições) e `"drinks"` (bebidas), pagas em moedas. Cada encomenda demora `orderHours` (1) hora de jogo e **chega à porta de casa**: uma pilha no chão da base junto ao ponto onde se aparece (`deliveryOffsetPx`), com aviso. Não dá XP; cancelar devolve as moedas.
- **Comida com efeito** (`buff: { effect, value, hours }` em `items.json`): pizza (+dano corpo a corpo), hambúrguer (+defesa), frango assado (+regeneração), bifana (+recursos extra), café (+XP), batido (correr gasta menos); sopa e limonada só matam a fome/sede. Os efeitos são os nomes dos talentos (`BUFF_EFFECTS` ⊂ `TALENT_EFFECTS`) e somam-se em `talentOf`; o mesmo efeito renova-se (não se acumula). Save v21: `player.buffs = [[efeito, valor, tick em que acaba]]`.

---

### 7.18 NPCs, missões e teletransporte pago

- **NPCs** (`src/data/npcs.json`; pontos `npc:<id>` nos mapas; bloqueiam com `NPC_FOOTPRINT`): técnico (ao lado de cada poste), comerciante (no Caminho 1, perto de casa; estação `merchant` com as mesmas trocas da banca e o **pergaminho de viagem**; falar com ele abre logo a loja, a não ser que tenha uma missão para dar ou entregar), velho, pescador, agricultor, médica, sargento. A ação contextual fala com eles (`npc:talk` → `DialogUI`). Falas longas dividem-se em páginas (`splitSpeech`, frases até ~90 letras): tocar fora do painel, no painel, em OK, Espaço ou Enter passa à seguinte; a última mostra a missão e as opções, e aí tocar fora (ou Espaço/Enter) sai. A tecla que fechou não volta logo a falar (`uiState.actionLocked` até se largar). Por cima: "!" (missão para dar) ou "?" (missão para entregar).
- Espaço/Enter numa conversa com uma missão para aceitar ou entregar faz isso logo (`DialogUI.confirm`), sem passar pelas falas; tocar fora continua a passar as falas.
- **Missões** (`src/data/quests.json`, lógica pura em `systems/quests/quests.ts`, estado em `core/Quests.ts`): progressivas (`requires`, `level`), com objetivos `reach` (ir a uma zona), `collect` (ter itens; entregam-se), `kill`, `talk`; prémios em XP, moedas e itens. O HUD mostra a missão ativa. Save v22: `player.quests = { active: { id: contagens }, done: [] }`.
...
- **Passar a borda não recomeça a cena** (parava o jogo 2 s ou mais no telemóvel): `ZoneScene.crossZone` desloca tudo o que está desenhado para as coordenadas da zona nova (`shiftWorld`; o Y-sort é `Y_SORT_BASE + y`), passa a lógica para ela e desenha as zonas que ficaram perto **uma por frame** (`streamNext`); as que ficam a mais de `NEIGHBOR_DROP_TILES` apagam-se. O HUD não se refaz (só o aviso com o nome e o botão Construir).
- **Divisórias naturais** (`npm run map:naturalize`, tiles `mound`, `rocks`, `log`, `waterfall`, `cliff` no fim do tileset): as linhas de vedações/rochedos/muros dos Caminhos e das bordas das zonas passaram a montes de erva, rochas e troncos caídos, com 1–2 tiles de espessura irregular; bordas viradas para o vazio são penhasco; uma barreira em cada três dos Caminhos é um riacho com ponte de tábuas e cascata. O script confirma que nenhuma abertura fica sem acesso (se ficasse, desfaz as irregularidades).
...
- Cada zona tem **pelo menos 1 saída** (`MIN_EXITS`; antes 2 — a base perdeu a de baixo, que abria o mapa-mundo: sai-se a pé pelo mundo contínuo) e uma área segura perto da entrada.
...
Os dois jogadores são **completos e autónomos**, como sozinhos: cada um com a sua personagem (mochila, hotbar, equipamento, vida/fome/sede, nível, receitas), fabrico, construção e viagens próprias — podem estar em zonas diferentes. O **mundo é o do anfitrião** e é partilhado: baús, peças construídas, estações, recursos apanhados, contentores, mochilas no chão, hordas e o relógio.

- [x] **Código de sessão**: "Convidar amigo" no menu de pausa mostra um código de **5 caracteres** (alfabeto de 33 símbolos sem `O`, `I`, `L`; ao escrever, O conta como 0 e I/L como 1: ~39 milhões de códigos). O código é o id do anfitrião no servidor de sinalização do PeerJS (`refugio-coop-<código>`), que **recusa ids em uso**: nunca há duas sessões ativas com o mesmo código. O amigo carrega em "Entrar com código" no menu inicial.
- [x] **Ligação**: WebRTC (DataChannel) com o **PeerJS** (MIT); a sinalização usa o servidor gratuito do PeerJS. `?peer=host:porta` na URL usa outro servidor (testes locais com o pacote `peer`).
- [x] **Personagem do convidado**: vem do save dele (ou uma nova, se não tiver) e volta para lá — a cada 10 s, ao sair e ao fechar a página grava-se no save **dele** (mantendo a posição que tinha no mundo dele). O que tirar dos baús do anfitrião leva para casa.
- [x] **Anfitrião autoritativo** (`src/net/`): o anfitrião corre uma **segunda `Simulation`** para o convidado, sobre o mesmo mundo (o `GameState` do convidado aponta para os mesmos objetos do mundo, com `player`/`unlocks`/`stats`/`tutorial` próprios e um `EventBus` próprio, `guestBus`). Corre dentro do tick do principal (o relógio, as estações e a horda avançam uma vez). **Na mesma zona**, as duas simulações partilham a zona (inimigos, peças, colisões: `Simulation.attach`/`syncLinks`, `Combat.link`, `Building.link`) e os inimigos atacam o jogador mais perto; **noutra zona**, o convidado tem a sua (com os seus inimigos).
- [x] **Ecrã do convidado**: anda localmente (manda a posição 15×/s); os painéis (mochila, fabrico, construção, viagens, pesca, tutorial) fazem logo a ação no ecrã dele e mandam-na ao anfitrião como comando (lista fechada em `GUEST_COMMANDS`). O anfitrião manda 10×/s um `frame` (vida/fome/sede, inimigos, o anfitrião se estiver na mesma zona, pesca) e o mundo inteiro (`snap`) quando muda (≤ 4×/s; de 3 em 3 s sem mudanças). O `snap` só se aplica quando o anfitrião já fez todos os comandos enviados (`ack`), e a posição só muda quando o anfitrião o muda de sítio (`warp`: ao entrar, ao morrer). A XP, os níveis e as estatísticas calculam-se só no anfitrião.
- [x] **Ao entrar**: o convidado aparece ao lado do anfitrião (não por cima), com aviso "Entraste no jogo de <nome>!" / "<nome> entrou no teu jogo!", e os nomes aparecem por cima dos dois bonecos.
- [x] **Morte do convidado**: como sozinho — a mochila fica no chão (no mundo do anfitrião) e ele reaparece na base.
- [x] **Dificuldade**: em co-op, os inimigos têm **1,5× vida e dano** (`coopEnemyMultiplier`; os que já estão na zona ajustam-se ao entrar o convidado).
- [x] O tempo não pára em co-op: a pausa e o mapa-mundo deixam o jogador "ausente" (os inimigos ignoram-no) e a velocidade fica em x1.
...
**Aceitação:** dois jogadores em redes diferentes entram com o código, recolhem, constroem e combatem juntos durante 30 min sem dessincronizar; nunca há duas sessões ativas com o mesmo código. *(Testado localmente com dois browsers e o servidor `peer`: baú partilhado, personagem gravada no save do convidado, zonas diferentes, inimigos partilhados na mesma zona, morte; falta testar entre redes diferentes.)*
...
| 2026-09-25 | Co-op com dois jogadores completos (substitui o "ajudante"): personagem própria do convidado, gravada no save dele; baús e resto do mundo partilhados; zonas independentes | Pedido do jogador: "exatamente como em single player, os 2 jogadores são autónomos e têm tudo disponível, com a diferença de poderem partilhar baús" |
| 2026-09-25 | No anfitrião, uma segunda `Simulation` para o convidado, sobre os mesmos objetos do mundo; ligada à principal na mesma zona | Reaproveita toda a lógica de um jogador (fabrico, construção, combate, progressão) sem a duplicar; sem mudar o formato do save |
...
| 2026-09-25 | Fonte Tiny5 com o tamanho arredondado à grelha dela (cada píxel da letra = píxeis inteiros do ecrã); sombra em vez de contorno | Pedido do jogador: letras e números mal definidos |
| 2026-09-25 | Save v20: corpos como pilhas no chão (abrem-se com a ação, duram 1 dia de jogo); máximo de 300 coisas no chão por zona; 5.º slot para a arma; aviso "Precisas de machado" faz o machado (toque ou Enter); zoom ao alto; viagens grátis sempre; co-op com nomes e O = 0 no código | Pedidos do jogador |
...
| 2026-09-26 | Véu da noite e montanhas posicionados pela vista deste frame (`ZoneScene.cameraView`: segue o jogador, com os limites), não pelo `worldView` do Phaser; o véu só se refaz quando a vista muda de tamanho e redesenha-se logo ao mudar o zoom | Pedido do jogador: ao mudar de zona ou de zoom depressa, à noite, via-se tudo claro durante 1 frame (o `worldView` só se atualiza ao desenhar) |
| 2026-09-26 | Conversas por páginas (tocar fora/OK/Espaço avança e, no fim, sai); marcadores no mapa (mochila, postes, missões, chefes); divisórias naturais com tiles novos | Pedidos do jogador: sair da conversa sem ser pelo ×, ver a mochila no mapa, e divisórias que "pareçam caminhos reais" em vez de cercas |
