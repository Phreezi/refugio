...
- **NPCs** (`src/data/npcs.json`; pontos `npc:<id>` nos mapas; bloqueiam com `NPC_FOOTPRINT`): técnico (ao lado de cada poste), comerciante (no Caminho 1, perto de casa; estação `merchant` com as mesmas trocas da banca e o **pergaminho de viagem**), velho, pescador, agricultor, médica, sargento. A ação contextual fala com eles (`npc:talk` → `DialogUI`). Falas longas dividem-se em páginas (`splitSpeech`, frases até ~90 letras): tocar fora do painel, no painel, em OK, Espaço ou Enter passa à seguinte; a última mostra a missão e as opções, e aí tocar fora (ou Espaço/Enter) sai. A tecla que fechou não volta logo a falar (`uiState.actionLocked` até se largar). Por cima: "!" (missão para dar) ou "?" (missão para entregar).
