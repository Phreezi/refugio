...
- **Corpos**: um inimigo derrotado fica no chão, na posição em que estava e **a cinzento** (`display/grayTexture.ts`), com os **drops** e as flechas que lhe acertaram e não partiram. É uma pilha no chão (`zones.<zona>.bags` com `corpse: <inimigo>`, save v20): abre-se com a ação ("Apanhar tudo" ou escolher) e, vazio, desaparece numa nuvem de fumo. Fica `corpseDays` (1) dia de jogo, também se o jogador sair da zona. No máximo `groundThingsMax` (300) coisas no chão por zona (corpos, pilhas, itens soltos): as mais antigas desaparecem (as mochilas da morte nunca).
