...
- Resolução **adaptável ao ecrã** com escala inteira (×2, ×3, ×4…) e `pixelArt: true`: escolhe-se o zoom inteiro (píxeis do dispositivo por píxel de jogo) cujo **lado curto** do jogo fica mais perto de **270 px** (≈ 17 tiles, o "zoom" estilo Stardew), e o outro lado enche o ecrã (proporção entre 9:21 e 21:9; fora disso, barras). Funciona **ao alto e ao baixo**: com o telemóvel na vertical vê-se uma área mais alta do que larga (ex.: 270×550). A vista tem sempre dimensões **pares**. Ex.: 1920×1080 → 480×270 ×4; janela 1530×790 → 510×262 ×3. Mínimo 216 px (abaixo disso, zoom fracionário). Valores em `DISPLAY` (`src/config.ts`).
  ...
  - **Fonte**: pixel, **Tiny5** (SIL OFL, com minúsculas e acentos; `public/assets/fonts/`, carregada antes do arranque por `src/display/fonts.ts`). A grelha da fonte é 1/8 do tamanho: o `Label` arredonda o tamanho (`pixelFontSize`) para cada píxel da letra ser um número inteiro de píxeis do ecrã — letras e números nítidos a qualquer zoom. Um só peso (sem negrito falso); o "contorno" é uma sombra de 1 píxel da letra. Títulos grandes em maiúsculas. *(Antes: Pixelify Sans, cuja grelha de 1/10 dava píxeis irregulares e um 2/5 que pareciam Z/S.)* Campos de texto (nome, código do co-op) são `<input>` do DOM com o aspeto do jogo (`src/ui/textInput.ts`).
